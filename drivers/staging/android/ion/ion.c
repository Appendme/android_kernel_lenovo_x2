...
static void *ion_buffer_kmap_get(struct ion_buffer *buffer)
{
	...
	if (WARN_ONCE(vaddr == NULL,
		...
		return ERR_PTR(-EINVAL);
	if (IS_ERR(vaddr)) {
                IONMSG("%s map kernel is failed addr = %pK.\n", __func__, vaddr);
		return vaddr;
        }
	buffer->vaddr = vaddr;
	buffer->kmap_cnt++;
				     ...
				     const char *name)
{
    ...
    if(IS_ERR_OR_NULL(client)) {
        IONMSG("%s client is error or null %pK.\n", __func__, client);
	return client;
    }
...
static int ion_vm_fault(struct vm_area_struct *vma, struct vm_fault *vmf)
{
	...
	mutex_unlock(&buffer->lock);
	if (ret) {
                IONMSG("%s vm insert pfn failed, vma = %pK, addr = %pK, pfn = %lu.\n", __func__, vma, vmf->virtual_address, pfn);
						...
						struct ion_handle *handle)
{
	...
	if (IS_ERR(dmabuf)) {
                IONMSG("%s dma buf export failed dmabuf is error %pK.\n", __func__, dmabuf);
		ion_buffer_put(buffer);
		return dmabuf;
	}
...
struct ion_handle *__ion_import_dma_buf(struct ion_client *client, int fd, int from_kern)
{
	...
	if (IS_ERR(dmabuf))
	{
		IONMSG("%s dma_buf_get fail fd=%d ret=%pK\n", __func__, fd, dmabuf);
...
static int ion_sync_for_device(struct ion_client *client, int fd)
{
	...
	dmabuf = dma_buf_get(fd);
	if (IS_ERR(dmabuf)) {
                IONMSG("%s dma_buf_get failed dmabuf is err %d, %pK.\n", __func__, fd, dmabuf);
		return PTR_ERR(dmabuf);
        }
...
static int ion_open(struct inode *inode, struct file *file)
{
	...
	if (IS_ERR(client)) {
                IONMSG("%s ion client create failed %pK.\n", __func__, client);
...
static int ion_debug_heap_show(struct seq_file *s, void *unused)
{
	...
	for (n = rb_first(&dev->clients); n; n = rb_next(n)) {
		...
		if (client->task) {
			...
			seq_printf(s, "%16.s(%16.s) %16u %16zu %pK\n", task_comm,
...
struct ion_handle* ion_drv_get_handle(struct ion_client* client, int user_handle, struct ion_handle* kernel_handle, int from_kernel)
{
	...
	if (from_kernel) {
		...
		if (IS_ERR_OR_NULL(handle)) {
			IONMSG("%s handle invalid, handle = %pK.\n", __FUNCTION__, handle);
			return ERR_PTR(-EINVAL);
		}
